...
static struct process_mapping current_map;
...
/* This one function is called recursively. */
static struct process_mapping load_elf_path(const char *path);
...
static void sigsegv(_unused int signal, siginfo_t *si, _unused void *uctxt) {
	...
	segfault = segment_lookup(&current_map, si->si_addr);
	if (segfault == NULL) {
		forward_sigsegv(si, uctxt);
		return;
	}
...
static void start_process(const struct process_mapping *map, int argc, char **argv, char **envp) {
	Elf64_Addr *sp;
	int envc = 0;
	size_t stacksize;
	int auxc = 0;
...
int main(int argc, char **argv, char **envp) {
	...
	current_map = load_elf_path(argv[1]);
	setup_sig_handler();
	start_process(&current_map, argc - 1, argv + 1, envp);
